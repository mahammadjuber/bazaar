...
export default class Subscriptions extends Component {
  ...
  ChangeDeliveryAddress=(CrData)=>{

    
    SubscriptionId=CrData['SubscriptionId'];

    fetch(APP_CONSTANTS.APP_API_URL+'getAddressBySubscription', {
      method: 'POST',
      headers: {
...
if(responseJson['status']=='Success'){
  ...
  navigate('ChangeAddress',{address_data:responseJson,SubscriptionId:SubscriptionId,UserId:this.state.LoggedInUserId});
}else{
...
if(responseJson['status']=='Success'){
  ...
  getSubscriptions=()=>{
    ...
    if(this.state.LoggedInUserId==null){
      Toast.show('Please login to see your orders!');
      const { navigate } = this.props.navigation;
      navigate('Login',{Origin:'Subscriptions'});
      return;
    }
    fetch(APP_CONSTANTS.APP_API_URL+'listOfSubscriptions', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
...
{IsDelivered==false ? <Image
  ...
  <Icon containerStyle={{paddingLeft:7}} 
        ...
        color='#ff5252'
        size={18}
        onPress={()=>this.cancelOrder(CrData)}
      />
  
  
            {IsDelivered==false ? <Text style={{paddingLeft:6}} onPress={()=>this.cancelOrder(CrData)}>Cancel</Text> : '' } 
     ...
     <Button 
       ...
       color:'white'}}
       onPress={()=>this.ViewSubPlans(CrData)}  title='View Plans'   
       buttonStyle={{
      ...
      {IsDelivered==false ?  <Button 
                ...
                color:'white'}}
               onPress={()=>this.ChangeDeliveryAddress(CrData)}
  ...
  render() {
    ...
    return (
      ...
      <View style={{backgroundColor:'white',height:'100%'}}>
        <CustomHeader page_name='My Orders'/>
