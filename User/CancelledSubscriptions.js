...
if(responseJson['status']=='Success'){
  ...
  getCncldSubscriptions=()=>{
  
    var SubscriptionsData=[];

    fetch(APP_CONSTANTS.APP_API_URL+'cancelledSubscriptions', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          .then((responseJson) => {
            ...
            for(i=0;i<Data.length;i++){
             ...
             IsRefunded=CrData['AmtWithUs']==0.00?true:false;
             ...
             Price=0.00;
             Subscribedon='N/A';//CrData['Subscribedon'];
             ExpireOn=CrData['ReqestedOn'];//CrData['ExpiryDate'];
...
<Text style={{paddingLeft:8}}>{Subscribedon}</Text> 
     ...
     <View style={{marginTop:-12,backgroundColor:'transparent',flexDirection:'row',paddingTop:10}}>
     <Image
  style={{width:22, height: 22}}
  source={require('../assets/images/icons8-cancel-22.png')}/>

  {IsDelivered==false ? <Text style={{paddingLeft:6}}>Cancelled On  {ExpireOn}</Text> : <Text style={{paddingLeft:6}}>Delivered on {ExpireOn}</Text> } 
...
:<Text></Text>}
            ...
            {IsRefunded==true ?  <Text> Refunded</Text> :<Text>Refund in Process.</Text> } 
  ...
  render() {
    ...
    return (
      ...
      <View style={{backgroundColor:'white',height:'100%'}}>
        <CustomHeader page_name='Cancelled'/>
